...
export const SUGESTOES_TIPOS = createI18nMessages({
    ajustarConfianca: (atual) => `${ICONES.feedback.dica} Use --confidence <num> para ajustar o limiar (atual: ${atual}%)`,
    revisar: (categoria) => `${ICONES.feedback.dica} Revise os casos ${categoria} manualmente`,
    anyEncontrado: [
    ...
    ajustarConfianca: (atual) => `${ICONES.feedback.dica} Use --confidence <num> to adjust threshold (current: ${atual}%)`,
    revisar: (categoria) => `${ICONES.feedback.dica} Review ${categoria} cases manually`,
    anyEncontrado: [
        ...
        case 'diagnosticar':
            if (!contexto.temProblemas) {
                if (contexto.countProblemas !== undefined) {
                    sugestoes.push(SUGESTOES_DIAGNOSTICO.poucoProblemas.replace('{count}', String(contexto.countProblemas)));
                }
            }
...
export function formatarSugestoes(sugestoes, titulo = i18n({ 'pt-BR': 'Sugestões', en: 'Suggestions' })) {
    if (sugestoes.length === 0)
        return [];
    const linhas = ['', `┌── ${titulo} ${'─'.repeat(50)}`.slice(0, 70)];
