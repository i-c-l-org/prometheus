...
function header({ projectNome, license, ptBr }) {
...
async function renderPackageBlock(pkgId, meta) {
    ...
    if (meta.publisher)
        lines.push(`Publicador: ${s(meta.publisher)}`);
    if (meta.email)
        lines.push(`Contato: ${s(meta.email)}`);
    if (meta.repository)
        lines.push(`Repositório: ${s(meta.repository)}`);
...
export async function generateNotices({ root = process.cwd(), ptBr = false, output } = {}) {
    ...
    if (!results) {
        try {
            const { stdout } = await execFile('npx', ['--yes', 'license-checker-rseidelsohn', '--production', '--json'], {
...
//# sourceMappingURL=generate-notices.js.map