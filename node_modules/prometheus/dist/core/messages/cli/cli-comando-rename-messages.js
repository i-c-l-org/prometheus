...
export const CliComandoRenameMensagens = createI18nMessages({
    ...
    erros: {
        falhaFlags: (erro) => `Failed to apply flags to rename command: ${erro}`,
    ...
    status: {
        inicio: (total) => `Starting variable renaming (${total} mappings)...`,
        atualizado: (rel) => `Updated: ${rel}`,
        concluido: (total) => `Renaming completed! ${total} files updated.`,
        conflito: (old, rel, nova, anterior) => `Mapping conflict for "${old}": ${rel} uses "${nova}", previous was "${anterior}" (last wins).`
    }
});
