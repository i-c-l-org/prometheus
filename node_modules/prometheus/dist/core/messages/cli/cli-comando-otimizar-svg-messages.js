...
export const CliComandoOtimizarSvgMensagens = createI18nMessages({
    ...
    erros: {
        falhaFlags: (erro) => `Failed to apply flags to optimize-svg command: ${erro}`,
        falhaOtimizar: (erro) => `Failed to optimize SVGs: ${erro}`
    },
    status: {
        titulo: 'OPTIMIZE SVG',
        linhaLogOtimizacao: (rel, original, otimizado, economia) => `${rel} — ${original} → ${otimizado} (−${economia})`,
        ...
        resumoCandidatos: (candidatos, economia, total) => `Candidates: ${candidatos} | Potential savings: ${economia} | Total SVGs read: ${total}`,
        concluidoWrite: (otimizados, candidatos) => `Optimization applied to ${otimizados}/${candidatos} files.`,
