...
export const CliComandoNamesMensagens = createI18nMessages({
    ...
    erros: {
        falhaFlags: (erro) => `Failed to apply flags to names command: ${erro}`,
        erroProcessar: (rel) => `[Warning] Error processing ${rel}`
    },
    status: {
        inicio: 'Starting variable name scan...',
        concluidoLegacy: (totalNomes, totalArquivos, saidaArquivo) => `Scan completed! ${totalNomes} variables in ${totalArquivos} files. Fragmented mapping in names/ and aggregated in ${saidaArquivo}.`,
        concluido: (totalNomes, totalArquivos) => `Scan completed! ${totalNomes} variables in ${totalArquivos} files. Mapping in names/ (mirrored structure).`
    }
});
