...
export function wrapComMetadados(data, schema, versao, descricao) {
    return {
        _metadata: {
            schema,
            versao,
...
export function getDescricaoCampo(caminho) {
    const parts = caminho.split('.');
    let current = JsonMensagens;
    ...
    if (typeof current === 'object' && current !== null && 'descricao' in current && typeof current.descricao === 'string') {
        return current.descricao;
    }
    return `${i18n({ 'pt-BR': 'Campo', en: 'Field' })}: ${caminho}`;
}
//# sourceMappingURL=json-messages.js.map