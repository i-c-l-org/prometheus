    ...
    aplicar: async (srcParam, relPath, _ast, fullCaminho) => {
        ...
        for (const tipoInseguro of tiposInseguros) {
            ...
            while ((match = tipoInseguro.padrao.exec(src)) !== null) {
                ...
                const linha = src.substring(0, position).split('\n').length;
                const lineContext = src.split('\n')[linha - 1]?.trim() || '';
                const mensagemCompleta = `${DetectorTiposInsegurosMensagens[tipoInseguro.chaveMensagem]()} | [SUGESTAO] ${DetectorTiposInsegurosMensagens[tipoInseguro.chaveSugestao]()} | [REVISAO] ${DetectorTiposInsegurosMensagens.revisaoManual}`;
                if (shouldSuppressOccurrence(tipoInseguro.tipo, relPath))
                    continue;
                ocorrencias.push({
                    tipo: tipoInseguro.tipo,
                    nivel: 'info',
                    mensagem: mensagemCompleta,
                    relPath,
                    linha,
