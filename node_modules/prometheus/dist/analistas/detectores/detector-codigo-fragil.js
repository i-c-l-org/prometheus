...
function detectarGodObjects(ast, fragilidades, limiteProps = 10) {
    traverse(ast.node, {
...
function detectarFeatureEnvy(ast, fragilidades, limiteAcessos = 5) {
    traverse(ast.node, {
        FunctionDeclaration(path) {
            const node = path.node;
...
function detectarDataClumps(ast, fragilidades, limiteParams = 3) {
    const parametrosGroups = new Map();
    traverse(ast.node, {
        FunctionDeclaration(path) {
