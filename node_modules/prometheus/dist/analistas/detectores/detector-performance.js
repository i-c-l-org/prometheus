...
export const analistaDesempenho = criarAnalista({
    ...
    aplicar: async (src, relPath, ast, _fullPath, _contexto) => {
        if (!src)
            return null;
...
function detectarConsoleEmProducao(linha, numeroLinha, problemas) {
...
function detectarMemoryLeakClosure(linha, numeroLinha, problemas) {
    const patterns = [
        /addEventListener\s*\(/,
        /setInterval\s*\(/,
        /setTimeout\s*\(/,
...
function detectarDOMManipulation(linha, numeroLinha, problemas) {
    const domPatterns = [
        /\.innerHTML\s*=/,
        /\.outerHTML\s*=/,
        /document\.write\s*\(/,
...
function detectarInefficientArray(linha, numeroLinha, problemas) {
    const patterns = [
...
function detectarPadroesPerformance(src, problemas, relPath, contexto) {
