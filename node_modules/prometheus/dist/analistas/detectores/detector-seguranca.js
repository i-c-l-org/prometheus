...
export const analistaSeguranca = criarAnalista({
    ...
    aplicar: async (src, relPath, ast, _fullPath, contexto) => {
        if (!src)
            return null;
...
function detectarSqlInjection(linha, numeroLinha, problemas, contexto) {
    const isFrontendOnly = contexto?.isFrontend && !contexto?.isBackend;
    if (linha.includes('URLSearchParams') || linha.includes('searchParams'))
        return;
...
function detectarMissingCSRF(linha, numeroLinha, problemas, contexto) {
    if (contexto && !contexto.isBackend)
        return;
...
function detectarSSRF(linha, numeroLinha, problemas, contexto) {
    const isFrontendOnly = contexto?.isFrontend && !contexto?.isBackend;
    if (isFrontendOnly)
        return;
...
function detectarXSSAvancado(linha, numeroLinha, problemas, contexto) {
...
function detectarOpenRedirect(linha, numeroLinha, problemas) {
...
function detectarPadroesPerigosos(src, relPath, problemas, contexto) {
...
function detectarProblemasAST(ast, problemas, _contexto) {
    try {
        traverse(ast.node, {
            NewExpression(path) {
