...
export async function exportarRelatorioSvgOtimizacao(params) {
    ...
    for (const e of entries) {
        if (!e || typeof e.relPath !== 'string')
            continue;
        if (!/\.svg$/i.test(e.relPath))
            continue;
        if (typeof e.content !== 'string')
            continue;
        if (!/<svg\b/i.test(e.content))
            continue;
