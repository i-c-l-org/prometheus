...
export const detectorEstrutura = {
  ...
  aplicar(_src: string, _relPath: string, _ast: NodePath<Node> | null, _fullPath?: string, contexto?: ContextoExecucao): TecnicaAplicarResultado {
    ...
    const sinais: SinaisProjeto & {
      ...
      temTestes: caminhosNorm.some(p => /(\.test\.|\.spec\.|__tests__|test\/|tests\/|spec\/)[^/]*\.(ts|js|tsx|jsx)$/i.test(p)),
      temCiCd: caminhosNorm.some(p => p.includes('.github/workflows/') || p.includes('.gitlab-ci.yml') || p.includes('azure-pipelines.yml') || p.includes('Jenkinsfile')),
      temDocker: caminhosNorm.some(p => p.toLowerCase().includes('dockerfile') || p.endsWith('.dockerfile') || p.includes('docker-compose'))
    };
    ...
    // Verificações de infraestrutura (apenas para projetos não-pequenos)
    if (!isProjetoPequeno && !isProjetoTeste) {
      if (!sinais.temTestes) {
        ocorrencias.push({
          tipo: 'estrutura-sem-testes',
          nivel: 'aviso',
          mensagem: DetectorEstruturaMensagens.semTestesUnitarios,
          origem: 'detector-estrutura',
          relPath: '[raiz do projeto]',
          linha: 0
        });
      ...
      if (!sinais.temCiCd) {
        ocorrencias.push({
          tipo: 'estrutura-sem-cicd',
          nivel: 'info',
          mensagem: DetectorEstruturaMensagens.semCiCd,
          origem: 'detector-estrutura',
          relPath: '[raiz do projeto]',
          linha: 0
        });
      ...
      if (!sinais.temDocker) {
        ocorrencias.push({
          tipo: 'estrutura-sem-docker',
          nivel: 'info',
          mensagem: DetectorEstruturaMensagens.semDockerfile,
          origem: 'detector-estrutura',
          relPath: '[raiz do projeto]',
          linha: 0
        });
