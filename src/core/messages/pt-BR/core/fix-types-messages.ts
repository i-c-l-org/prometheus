...
export const MENSAGENS_INICIO = createI18nMessages({
  ...
  modo: (dryRun: boolean) => `${dryRun ? ICONES_ACAO.analise : ICONES_ACAO.correcao} Mode: ${dryRun ? 'Analysis (dry-run)' : 'Apply corrections'}`
});
...
export const DICAS = createI18nMessages({
  ...
  ajustarConfianca: (atual: number) => `${ICONES_FEEDBACK.dica} Use --confidence <num> to adjust threshold (current: ${atual}%)`,
...
export const MENSAGENS_CLI_CORRECAO_TIPOS = createI18nMessages({
  ...
  linhaEmBranco: '',
  erroExecutar: (mensagem: string) => `Error executing fix-types: ${mensagem}`,
  linhaResumoTipo: (texto: string) => `  ${texto}`,
  ...
  verboseAnyDetectado: (arquivo: string, linha: string) => `  ${ICONES_TIPOS.any} ${arquivo}:${linha} - any detected (correction recommended)`,
  verboseAsAnyCritico: (arquivo: string, linha: string) => `  ${ICONES_TIPOS.corrigir} ${arquivo}:${linha} - "as any" detected (CRITICAL - mandatory correction)`,
  ...
  altaConfiancaTitulo: (qtd: number) => `${ICONES_DIAGNOSTICO.stats} ${qtd} high-confidence corrections (≥85%):`,
  altaConfiancaLinha: (relPath: string | undefined, linha: string, confianca: number) => `  ${ICONES_TIPOS.corrigir} ${relPath}:${linha} (${confianca}%)`,
  altaConfiancaDetalhe: (texto: string) => `     └─ ${texto}`,
  altaConfiancaMais: (qtd: number) => `  ... and ${qtd} more corrections`,
