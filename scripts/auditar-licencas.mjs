...
async function collectLicenses(deps) {
  ...
  for (const name of deps) {
    ...
    const packageData = await readJSONSafe(pkgPath);
    if (!packageData) {
      results.push({ name, version: 'unknown', license: 'UNKNOWN', path: pkgPath, kind: 'npm' });
      continue;
    }
    const license = normalizeLicense(packageData.license || packageData.licenses);
    results.push({
      name,
...
async function main() {
  ...
  const patterns = [
    { name: 'GPL/AGPL/LGPL', rx: /\bGPL(?:-[0-9]+(?:\.[0-9]+)?)?(?:\+)?\b|\bAGPL(?:-[0-9]+(?:\.[0-9]+)?)?(?:\+)?\b|\bLGPL(?:-[0-9]+(?:\.[0-9]+)?)?(?:\+)?\b/i },
    { name: 'Copyleft', rx: /\bcopyleft\b/i },
    {
      name: 'CLA/cessão',
      rx: /(?:^|[^-\w])(Contributor License Agreement|assignment of copyright|transfer[eê]ncia de propriedade|cess[aã]o de propriedade|assignment of rights|proprietary license)(?:[^-\w]|$)/i,
    },
    {
      name: 'Proprietary-only',
      rx: /(?:^|[^-\w])(proprietary software|all rights reserved|uso n[aã]o autorizado|distribui[cç][aã]o proibida)(?:[^-\w]|$)/i,
    },
  ];
